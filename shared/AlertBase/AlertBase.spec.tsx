...
describe('AlertBase Component', () => {
  const mockAlerts = [
    { type: AlertType.SUCCESS, message: 'Success message' },
    { type: AlertType.ERROR, message: 'Error message' },
  ];
  ...
  test('renders AlertBase component with alerts', () => {
    ...
    const alertItems = screen.getAllByTestId((id) => id.startsWith('alert-item'));
  ...
  test('closes an alert when the close button is clicked', () => {
    ...
    const closeButtons = screen.getAllByTestId((id) => id.startsWith('alert-close'));
    expect(closeButtons.length).toBe(mockAlerts.length);

    fireEvent.click(closeButtons[0]);

    const remainingAlerts = screen.getAllByTestId((id) => id.startsWith('alert-item'));
