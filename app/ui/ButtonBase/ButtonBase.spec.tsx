...
describe('ButtonBase Component', () => {
  ...
  it('calls onClick handler when the button is clicked', () => {
    ...
    const buttonText = 'Click me';
    const { getByText } = render(<ButtonBase onClick={onClickMock}>{buttonText}</ButtonBase>);
    const button = getByText(buttonText);
  ...
  it('renders a disabled button', () => {
    const buttonText = 'Click me';
    const { getByText } = render(<ButtonBase disabled>{buttonText}</ButtonBase>);
    const button = getByText(buttonText) as HTMLButtonElement;
    expect(button).toBeDisabled();
  });

  it('renders a large secondary outlined button with loading spinner', () => {
    const buttonText = 'Click me';
    const { getByText, container } = render(
      <ButtonBase large secondary outline isLoading>
        {buttonText}
      </ButtonBase>
    );
